...
class EvaluateManager(object):
    ...
    Attributes:
        _descriptor_model, sklearn.decomposition.PCA: Model for PC1 computation.
    ...
    def __init__(self, paths):
        ...
        full_path = os.path.join(self._paths['descriptor_model'],
                                'descriptor_model.pkl')
        self._descriptor_model = pickle.load(open(full_path, 'r'))
    ...
    def _loadFeature(self, conv_path):
        ...
        for i, name_i in enumerate(name_list):
            ...
            # DDT spatial weight.
            deep_descriptor = np.reshape(conv_i, (D, H * W)).T
            pc1_i = self._descriptor_model.transform(deep_descriptor)
            ...
            # CroW channel weight.
            Q = np.sum(conv_i > 0, axis=(1, 2), keepdims=True) / (H * W)
            ...
            # Weighted-pooling.
            descriptor_i = np.sum(ddt_spatial_w * (crow_channel_w * conv_i),
                                  axis=(1, 2))[np.newaxis, :]
...
def main():
    ...
    paths = {
        ...
        'descriptor_model': test_data_root,
    }
    for k in paths:
        if k != 'compute_ap':
