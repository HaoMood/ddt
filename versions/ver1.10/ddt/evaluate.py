...
class EvaluateManager(object):
    ...
    def _loadFeature(self, pc1_path, pool5_path):
        ...
        for i, name_i in enumerate(name_list):
            ...
            # Sum-pooled channel weight.
            Q = np.sum(pool5_i, axis=(1, 2))
            assert Q.shape == (512,)
            if np.sum(Q) <= 1e-3:
                print("%s's channel weight is 0." % name_i)
                crow_channel_w = np.ones((512, 1, 1))
            else:
                crow_channel_w = sklearn.preprocessing.normalize(
                    Q[np.newaxis, :]).T
                crow_channel_w = np.sqrt(crow_channel_w)
                crow_channel_w = crow_channel_w[:, :, np.newaxis]
                assert crow_channel_w.shape == (512, 1, 1)
...
def main():
    ...
    project_root = '/data/zhangh/project/scpp/'
    test_data_root = os.path.join('/data/zhangh/project/scpp/data/', args.test)
    via_data_root = os.path.join('/data/zhangh/project/scpp/data/', args.via)
    paths = {
