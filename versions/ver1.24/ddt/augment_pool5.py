...
class VGGManager(object):
    ...
    def getPool5(self, image_path, pool5_path):
        ...
        for i, name_i in enumerate(name_list):
            ...
            name_list = name_wo_ext_i, name_wo_ext_i + '_res'
    ...
    def _loadImage(self, image_full_path):
        ...
        new_w, new_h = [int(d * scale) for d in rgb_image.size]
        rgb_image_res = rgb_image.resize((new_w, new_h))
        # Image to np.ndarray.
        input_i_res = np.array(rgb_image_res, dtype=np.float32)  # HWD format
        input_i_res = input_i_res[:, :, ::-1]  # BGR order
        # Subtract mean pixel values of VGG training set.
        input_i_res -= np.array([104.00698793, 116.66876762, 122.67891434])
        input_i_res = input_i_res.transpose((2, 0, 1))  # DHW format
        return input_i, input_i_res
