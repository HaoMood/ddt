...
class DDTManager(object):
    ...
    def fit(self):
        ...
        for i, f in enumerate(pool5_list):
            ...
            # Load PC1 projections.
            pc1_i = np.load(os.path.join(self._paths['via_all_pc1'], f))
            pc1_i = np.maximum(pc1_i, 0)
            assert pc1_i.shape == (H * W, 1)
            # Judge by area.
            pc1_i_nonzero = np.sum(pc1_i > 0) / (H * W)
            if pc1_i_nonzero > 0.3:
        ...
        print('Total number of ignore numbers', ignore_count)
