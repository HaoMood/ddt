    ...
    def _loadFeature(self, pc1_path, pool5_path):
        ...
        for i, name_i in enumerate(name_list):
            ...
            if np.sum(pc1_i) <= 1e-3:
                ...
                ddt_spatial_w = np.reshape(pc1_i, (H, W))
                structure = np.array([[1, 1, 1], [1, 1, 1], [1, 1, 1]])
                labeled_array, num_features = scipy.ndimage.label(
                    input=ddt_spatial_w, structure=structure)
                sum_ = scipy.ndimage.sum(input=(ddt_spatial_w > 0),
                                         labels=labeled_array,
                                         index=range(1, num_features + 1))
                # Get the label index of largest connected component.
                largest_label_index = np.argmax(sum_) + 1
                largest_label = np.zeros(num_features + 1, np.uint8)
                largest_label[largest_label_index] = 1
                ddt_spatial_w *= largest_label[labeled_array]

                # l2,sqrt-normalize.
                ddt_spatial_w = np.reshape(ddt_spatial_w, (1, H * W))
                ddt_spatial_w = sklearn.preprocessing.normalize(ddt_spatial_w)
            ...
            # if np.sum(S) == 0:
            #     crow_spatial_w = np.ones((1, H, W))
            # else:
            #     crow_spatial_w = np.sqrt(sklearn.preprocessing.normalize(
            #         np.reshape(S, (1, H * W))))
            #     crow_spatial_w = np.reshape(crow_spatial_w, (1, H, W))
            ...
            # Weighted-pooled descriptors.
            # CroW.
            # descriptor_i = np.sum(crow_spatial_w * (crow_channel_w * pool5_i),
            #                       axis=(1, 2))[np.newaxis, :]
            # DDT only.
            ...
            descriptor_i = np.sum(ddt_spatial_w * (crow_channel_w * pool5_i),
                                  axis=(1, 2))[np.newaxis, :]
            # DDT + channel weighting, plus CroW
            # descriptor_i = np.zeros((1, 512 * 2))
            # descriptor_i[0, :512] = np.sum(
            #     ddt_spatial_w * (crow_channel_w * pool5_i),
            #     axis=(1, 2))[np.newaxis, :]
            # descriptor_i[0, 512:] = np.sum(
            #     crow_spatial_w * (crow_channel_w * pool5_i),
            #     axis=(1, 2))[np.newaxis, :]
...
def main():
    ...
    paths = {
        ...
        'via_all_pc1': os.path.join(via_data_root, 'pc1/all/'),
