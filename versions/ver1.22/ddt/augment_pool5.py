...
class VGGManager(object):
    ...
    def getPool5(self, image_path, pool5_path):
        ...
        for i, name_i in enumerate(name_list):
            ...
            name_list = name_wo_ext_i, name_wo_ext_i + '_rot'
    ...
    def _loadImage(self, image_full_path):
        ...
        # Rotate
        rgb_image_rot = rgb_image.rotate(random.uniform(-10, 10))
        # Image to np.ndarray.
        input_i_rot = np.array(rgb_image_rot, dtype=np.float32)  # HWD format
        input_i_rot = input_i_rot[:, :, ::-1]  # BGR order
        # Subtract mean pixel values of VGG training set.
        input_i_rot -= np.array([104.00698793, 116.66876762, 122.67891434])
        input_i_rot = input_i_rot.transpose((2, 0, 1))  # DHW format
        return input_i, input_i_rot
