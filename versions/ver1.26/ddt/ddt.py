...
class DDTManager(object):
    ...
    def fit(self):
        ...
        print('Fit PCA.')
        self._pca_model = sklearn.decomposition.PCA(n_components=2)
        self._pca_model.fit(all_pool5)
        eigenvalues = self._pca_model.explained_variance_
        np.save(os.path.join(self._paths['test_eigenvalues'], 'eigenvalues'),
                eigenvalues)
