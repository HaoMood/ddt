...
class EvaluateManager(object):
    ...
    def _loadFeature(self, pc1_path, pool5_path):
        ...
        for i, name_i in enumerate(name_list):
            ...
            # DDT spatial weight.
            pc_i = np.load('%s.npy' % os.path.join(self._paths[pc1_path],
                                                    name_i))
            assert pc_i.shape == (H * W, 2)
            eigenvalues = np.load(os.path.join(self._paths['test_eigenvalues'],
                                               'eigenvalues.npy'))
            assert eigenvalues.shape == (2,)
            eigenvalues = np.reshape(eigenvalues, (1, 2))

            pc_i = np.maximum(pc_i, 0)
            if np.min(np.sum(pc_i, axis=0)) <= 1e-3:
                print("%s's DDT weight is 0." % name_i)
                ddt_spatial_w = np.zeros((1, H, W))
            else:
                sklearn.preprocessing.normalize(eigenvalues, copy=False)
                ddt_spatial_w = np.sum(pc_i, axis=1,
                                       keepdims=True)
                ddt_spatial_w *= (pc_i[:, 0:1] > 0)
                assert ddt_spatial_w.shape == (H * W, 1)
                ddt_spatial_w = sklearn.preprocessing.normalize(ddt_spatial_w.T)
                ddt_spatial_w = np.sqrt(ddt_spatial_w)
                ddt_spatial_w = np.reshape(ddt_spatial_w, (1, H, W))
                # for j in range(2):
                #     pcj_i = pc_i[:, j:j+1]
                #     pcj_i = np.maximum(pcj_i, 0)
                #     assert pcj_i.shape == (H * W, 1)
...
def main():
    ...
    paths = {
        ...
        'test_eigenvalues': os.path.join(test_data_root, 'pc1-2/'),
        'test_all_pc1': os.path.join(test_data_root, 'pc1-2/all/'),
        'test_crop_pc1': os.path.join(test_data_root, 'pc1-2/crop/'),
        'test_full_pc1': os.path.join(test_data_root, 'pc1-2/full/'),
        'via_all_pc1': os.path.join(via_data_root, 'pc1-2/all/'),
